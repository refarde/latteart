...
( function() {
	...
	ExifTags = EXIF.Tags = {			// version tags
	...
	StringValues = EXIF.StringValues = {
		...
		Flash: {
			...
			0x004D: "Flash fired, compulsory flash mode, " +
					"red-eye reduction mode, return light not detected",
			0x004F: "Flash fired, compulsory flash mode, " +
	...
	function getImageData( img, callback ) {
		var http, fileReader, arrayBuffer,
	...
	function findEXIFinJPEG( file ) {
		...
		_log( "Got file of length " + file.byteLength );

		if ( ( dataView.getUint8( 0 ) !== 0xFF ) || ( dataView.getUint8( 1 ) !== 0xD8 ) ) {
			_log( "Not a valid JPEG" );
		...
		while ( offset < length ) {
			if ( dataView.getUint8( offset ) !== 0xFF ) {
				_log( "Not a valid marker at offset " + offset +
					", found: " + dataView.getUint8( offset ) );
	...
	function readTags( file, tiffStart, dirStart, strings, bigEnd ) {
		...
		for ( i = 0; i < entries; i++ ) {
			...
			if ( !tag ) {
				_log( "Unknown tag: " + file.getUint16( entryOffset, !bigEnd ) );
			}
	...
	*/
	...
	function readThumbnailImage( dataView, tiffStart, firstIFDOffset, bigEnd ) {
		...
		if ( !IFD1OffsetPointer ) {
			_log( "******** IFD1Offset is empty, image thumb not found ********" );
			return {};
		} else if ( IFD1OffsetPointer > dataView.byteLength ) { // this should not happen
			_log( "******** IFD1Offset is outside the bounds of the DataView ********" );
			return {};
		}

		_log( "*******  thumbnail IFD offset (IFD1) is: " + IFD1OffsetPointer );
	...
	function readEXIFData( file, start ) {
		if ( getStringFromDB( file, start, 4 ) !== "Exif" ) {
			_log( "Not valid EXIF data! " + getStringFromDB( file, start, 4 ) );
			return false;
		}
		...
		if ( file.getUint16( tiffOffset ) === 0x4949 ) {
			...
			_log( "Not valid TIFF data! (no 0x4949 or 0x4D4D)" );
			return false;
		}

		if ( file.getUint16( tiffOffset + 2, !bigEnd ) !== 0x002A ) {
			_log( "Not valid TIFF data! (no 0x002A)" );
			return false;
		}
